...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ap = None
        self.annotation_path = annotation_path#os.path.join(args.basic.path_dataset,annotation_path)
        self.prediction = None
        if args.feature_type == 'I3D':
            self.factor = 25.0 / 16.0
        elif args.feature_type == 'UNT':
            self.factor = 10.0 / 4.0
        else:
            self.factor = 30.0 / 15.0
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        if 'ActivityNet1.3' not in annotation_path:
        ...
        # else:
        subset_ind=[ss in self.subset for ss in subset]
        ...
        for i in range(len(gtsegments)):
            for j in range(len(gtsegments[i])):
                ...
                t_start_lst.append(round(gtsegments[i][j][0]*self.factor))
                t_end_lst.append(round(gtsegments[i][j][1]*self.factor))
                try:
                    label_lst.append(str2ind(gtlabels[i][j], self.classlist))
                except:
                    print(videoname[i],gtlabels[i],gtsegments[i])
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        for tidx, tiou_thr in enumerate(tiou_thresholds):
            for jdx in tiou_sorted_idx:
                ...
                tp[tidx, idx] = 1
                lock_gt[tidx, this_gt.loc[jdx]["index"]] = idx   # 这里的“index”是因为在得到this_gt的时候没有reset_index，所以“index”那一列保存的是this_gt中的每一行在原本ground_truth_gbvn中的索引
                break
