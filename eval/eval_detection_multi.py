...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        #                     if len(s) >0 ]
        ## =====select the videos with multiple foreground action classes and with gtsegements====
        self.idx_to_take=[]
        for i,(s,gtl) in enumerate(zip(gtsegments,gtlabels)):
            if len(s)>0:
                if len(list(set(gtl)))>=2:
    ...
    def wrapper_compute_average_precision(self):
        ...
        for i, x in enumerate(self.activity_index.values()):
            ap[:, i] = results[i]
    ...
    def evaluate(self):
        ...
        # self.prediction=self.prediction.iloc[self.idx_to_take]
        # select the videos with multiple classes
        select_video_names=self.ground_truth['video-id'].tolist()
        select_video_names=list(set(select_video_names))
        self.prediction=self.prediction.loc[self.prediction['video-id'].isin(select_video_names)]
        ...
        if self.verbose:
            ...
            print('\tSelect videos numbers {}'.format(len(select_video_names)), select_video_names )
